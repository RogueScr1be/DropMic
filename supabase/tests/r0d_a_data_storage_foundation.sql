...
select col_default_is('public', 'attempt_metrics', 'derived_metrics', '{}', 'derived metrics default to an object');
select col_default_is('public', 'analysis_runs', 'status', 'requested', 'analysis starts requested');
...
select ok(
  has_function_privilege('authenticated', 'public.request_quick_read(uuid,text,text)', 'EXECUTE')
    and not has_function_privilege('anon', 'public.request_quick_read(uuid,text,text)', 'EXECUTE'),
  'request boundary is authenticated-only'
);
select ok(
  has_function_privilege('authenticated', 'public.advance_analysis_run(uuid,public.analysis_run_status,text)', 'EXECUTE')
    and not has_function_privilege('anon', 'public.advance_analysis_run(uuid,public.analysis_run_status,text)', 'EXECUTE'),
  'status transition boundary is authenticated-only'
);
select ok(
  ...
  'account deletion remains authenticated-only'
);
