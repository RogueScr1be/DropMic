-- R0F-B1A-R2 local rollback only. Run with zero Mic Flow completion rows.
...
begin
  ...
  if found then
    ...
    select * into state_row from public.mic_flow_state where owner_id = current_user_id for update;
    if not found then
      return jsonb_build_object('status', 'unavailable');
    end if;
    select exists (
      select 1 from public.billing_entitlements
      where owner_id = current_user_id and entitlement_key = 'plus'
        and ((status = 'active' and expires_at > server_completed_at)
    ...
    if state_row.saves_available > save_cap then
      ...
      update public.mic_flow_state set saves_available = save_cap where owner_id = current_user_id;
    end if;
  ...
  if found then
    ...
    insert into public.mic_flow_state (owner_id) values (current_user_id) returning * into state_row;
    next_saves := least(state_row.saves_available, save_cap);
  end if;
  ...
  insert into public.mic_flow_completions (
    completion_id, owner_id, mode, topic_id, selected_duration_seconds,
    completed_duration_seconds, completed_at, timezone, qualifying_day
  ) values (
    trim(p_completion_id), current_user_id, trim(p_mode), trim(p_topic_id),
    p_selected_duration_seconds, p_completed_duration_seconds, server_completed_at,
    trim(p_timezone), local_day
  ) on conflict do nothing returning * into inserted_completion;
...
alter table public.mic_flow_completions
  drop constraint if exists mic_flow_completions_pkey;

alter table public.mic_flow_completions
  add constraint mic_flow_completions_pkey primary key (completion_id);
