-- R0F-B1B-R6: exact PostgreSQL catalog timezone validation.
-- Replace only the deployed RPC definitions; CREATE OR REPLACE preserves owners/grants.
-- Completion ordering and snapshot read-only behavior are unchanged.
...
begin
  ...
  if found then
    if existing_completion.mode <> trim(p_mode)
       ...
       or existing_completion.timezone <> p_timezone then
      return jsonb_build_object('status', 'unavailable');
    end if;
  ...
  if found then
    ...
    if state_row.last_completed_at is not null and state_row.timezone is not null then
      if not exists (select 1 from pg_catalog.pg_timezone_names where name = state_row.timezone) then
        return jsonb_build_object('status', 'unavailable');
      end if;
