...
- R0D implementation must enforce the three-reads/day quota server-side, make retries idempotent per attempt/run, delete audio through the Storage API, and keep bounded cleanup for expired audio and transcripts; automated 24-hour/30-day scheduling is the deferred D2-A Ops gate.
...
- Storage deletion is not proven by a successful API response alone: use the shared verifier, inspect exact returned paths, and confirm missing or ambiguous paths through metadata-only absence checks; never download audio or delete `storage.objects` rows directly.
- Secret-bearing database requests must use one in-memory construction layer with bounded execution; never compose credentials across nested shell-escaping layers.
- D2-A Core is the local MVP privacy slice: verified successful-audio deletion and bounded cleanup remain active, while Vault, Cron, automatic TTL enforcement, and live retention acceptance are deferred to D2-A Ops immediately before external testing.
...
- Expo audio mode is process-global. Automatic Solari and completion sounds must request `playsInSilentMode: false`; explicit Saved Drop playback requests `playsInSilentMode: true`, and recording uses its required recording mode.
- Serialize audio-mode operations and restore automatic mode after recording and explicit-playback boundaries, including failure cleanup. Nonessential audio failures remain non-blocking.
- QA7-I automated validation passed. Physical iPhone mute-switch validation is a deferred release check, not a source-control or full-device acceptance claim; no deployment is authorized until it passes. VoiceOver is waived from the current demo gate because the observed problem was device-level, and authorized live OTP remains a release check.
