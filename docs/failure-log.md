...
## 2026-09-17 — Automatic audio mode did not respect the physical silent switch

- Symptom: automatic Solari clacks and the completion bell were configured with `playsInSilentMode: true`, so the process-global audio session could bypass the iPhone silent switch. Explicit Saved Drop playback intentionally requires the opposite behavior.
- Root cause: Expo audio mode is process-global and the automatic, recording, and explicit-playback boundaries did not share serialized mode ownership or consistently restore the automatic mode.
